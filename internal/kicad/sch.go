...
// Bommable reports whether this symbol belongs on a BOM: power rails and anything
// the designer excluded do not.
...
// parseSymbol reads one schematic symbol. The reference comes from the instances block,
// not the Reference property: that is what KiCad annotates, and in a hierarchical
// design it is the only place the per-instance name lives.
...
// dedupeUnits collapses a multi-unit symbol — an opamp drawn as four gates is one BOM
// line. The unit carrying the footprint wins.
...
// BOMSymbols are the symbols belonging on a BOM, DNP included: whether a DNP part
// should be there is the caller's call, and the diff reports on it.
